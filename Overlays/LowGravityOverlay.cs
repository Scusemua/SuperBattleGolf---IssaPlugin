...
namespace IssaPlugin.Overlays
{
    ...
    /// Bar textures are generated at screen-pixel resolution so rounded corners are never
    /// distorted by scaling. They are regenerated when Screen.width changes.
    ///
    /// Slot assignment: slot 0 (bottom) when alone; slot 1 (above the Freeze bar) when
    /// FreezeItem.IsFrozen is also true, so the two bars never overlap.
    ///
    ...
    public class LowGravityOverlay : MonoBehaviour
    {
        ...
        // Purple fill colour
        private static readonly Color FillColor = new Color(0.5f, 0.15f, 0.9f, 0.9f);
        ...
        /// <param name="duration">Total duration in seconds (required when active = true).</param>
        ...
        private void OnGUI()
        {
            ...
            // Slot 1 when Freeze is simultaneously active so bars don't overlap; slot 0 otherwise
            int   slot = FreezeItem.IsFrozen ? 1 : 0;
            ...
            GUI.Label(new Rect(barX, barY, barW, barH), $"LOW GRAVITY  {remaining:F1}s", _labelStyle);
        }
