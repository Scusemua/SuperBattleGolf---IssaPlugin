...
namespace IssaPlugin
{
    public class PlaceableWallConfig
    {
        ...
        public PlaceableWallConfig(ConfigFile cfg, GlobalConfig global)
        {
            ...
            MaxPlacementDistance = cfg.Bind(
                Section,
                "MaxPlacementDistance",
                20f,
                "Maximum distance (units) from the player's camera at which a wall can be placed."
            );
            MinHoleDistance = cfg.Bind(
                Section,
                "MinHoleDistance",
                3f,
                "Minimum XZ distance (units) from the hole centre that a wall may be placed. Prevents blocking the hole entirely. Set to 0 to disable the check."
            );
            HealthPoints = cfg.Bind(
                Section,
                "HealthPoints",
                3f,
                "Number of golf club or baseball bat hits required to destroy the wall. Rocket hits always destroy in one shot."
            );
            VelocityImpactFactor = cfg.Bind(
                Section,
            ...
            TorsionMultiplier = cfg.Bind(
                Section,
                "TorsionMultiplier",
                0.002f,
                "Degree to which torque (spin) damages the bricks of the placeable wall. Default: 0.002."
            );
            RotationStep = cfg.Bind(
                Section,
                "RotationStep",
                45f,
                "Degrees the wall rotates per scroll-wheel tick during the placement preview. Scroll up rotates clockwise, scroll down rotates counter-clockwise."
            );
            DebrisLifetime = cfg.Bind(
                Section,
                "DebrisLifetime",
                30f,
                "Seconds before detached wall debris (bricks/pillars) are automatically destroyed."
            );
            RocketExplosionForce = cfg.Bind(
                Section,
