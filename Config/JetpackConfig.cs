    ...
    public class JetpackConfig
    {
        ...
        public JetpackConfig(ConfigFile cfg, GlobalConfig global)
        {
            ...
            SpawnWeight = global.BindSpawnWeight(
                ...
                "JetpackSpawnWeight",
                10f,
                "Override spawn weight for the Jetpack."
            );
            FuelPerUse = cfg.Bind(
                Section,
                "FuelPerUse",
            ...
            ThrustForce = cfg.Bind(
                Section,
                "ThrustForce",
                35f,
                "Upward acceleration (m/s²) applied each physics step while thrusting. Uses ForceMode.Acceleration."
            );
            UseJumpToActivate = cfg.Bind(
                Section,
                "UseJumpToActivate",
                true,
                "When enabled, pressing Space (Jump) activates the jetpack even if it is not the active item. The fuel gauge is visible whenever the jetpack is in inventory."
            );
        }
    }
}
