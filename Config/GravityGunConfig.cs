...
namespace IssaPlugin
{
    public class GravityGunConfig
    {
        ...
        public GravityGunConfig(ConfigFile cfg, GlobalConfig global)
        {
            ...
            TetherRadius = cfg.Bind(Section, "TetherRadius", 20f, "Distance (units) at which the target orbits the wielder while tethered.");
            TetherDuration = cfg.Bind(Section, "TetherDuration", 5f, "Maximum tether duration in seconds before automatic release.");
            SpringForce = cfg.Bind(Section, "SpringForce", 12f, "Spring stiffness coefficient; higher values pull the target faster.");
            MaxPullSpeed = cfg.Bind(Section, "MaxPullSpeed", 18f, "Maximum velocity change (m/s) applied to the target per physics tick.");
            InputSendInterval = cfg.Bind(Section, "InputSendInterval", 0.05f, "Interval (seconds) between aim-tick messages sent to the server (~20 Hz).");

            SpawnWeight = global.BindSpawnWeight(cfg, 121, "GravityGunSpawnWeight", 10f, "Override spawn weight for the Gravity Gun.");
        }
    }
}
