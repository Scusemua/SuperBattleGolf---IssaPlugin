...
namespace IssaPlugin.Patches
{
    ...
    static class MatchSetupRulesSpawnChanceUpdatedPatch
    {
        static MethodBase TargetMethod() =>
            AccessTools.Method(typeof(MatchSetupRules), "SpawnChanceUpdated",
    ...
    {
        static MethodBase TargetMethod() =>
            AccessTools.Method(typeof(MatchSetupRules), "Update");

        private static readonly FieldInfo CurrentItemPoolDirtyField =
            AccessTools.Field(typeof(MatchSetupRules), "currentItemPoolDirty");
        private static readonly MethodInfo GetCurrentItemPoolMethod =
            AccessTools.Method(typeof(MatchSetupRules), "GetCurrentItemPool");
        private static readonly FieldInfo CurrentItemPoolIndexField =
            AccessTools.Field(typeof(MatchSetupRules), "currentItemPoolIndex");
        private static readonly FieldInfo SpawnChanceWeightsField =
            AccessTools.Field(typeof(MatchSetupRules), "spawnChanceWeights");
        private static readonly FieldInfo SpawnChanceSlidersField =
            AccessTools.Field(typeof(MatchSetupRules), "spawnChanceSliders");
        ...
        static bool Prefix(MatchSetupRules __instance)
        {
            ...
            var weights = (IDictionary<MatchSetupRules.ItemPoolId, float>)SpawnChanceWeightsField.GetValue(__instance);
            ...
            foreach (var chance in currentPool.SpawnChances)
            {
                ...
                if (sliderIdx < 0 || sliderIdx >= sliders.Count)
                    continue;
                var key = MatchSetupRules.ItemPoolId.Get(poolIndex, chance.item);
                float pct = (total > float.Epsilon && weights.ContainsKey(key))
                    ? (weights[key] / total)
                    : 0f;
