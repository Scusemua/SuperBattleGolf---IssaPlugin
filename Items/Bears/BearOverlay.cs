...
namespace IssaPlugin.Overlays
{
    ...
    public class BearOverlay : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            ...
            if (_killFlashAlpha > 0f)
            {
                GUI.color = new Color(BearBrown.r, BearBrown.g, BearBrown.b,
            ...
            _timerStyle.normal.textColor = lowTime
                ? new Color(WarningRed.r, WarningRed.g, WarningRed.b, 0.7f + 0.3f * Mathf.Sin(_time * 8f))
                : new Color(BearBrown.r, BearBrown.g, BearBrown.b, 0.85f);

            int secs = Mathf.CeilToInt(remaining);
            GUI.Label(new Rect(sw - 200f, 12f, 185f, 32f),
                      $"⏱  {secs / 60:D2}:{secs % 60:D2}", _timerStyle);
        ...
        private void DrawBottomBar(float sw, float sh)
        {
            ...
            GUI.Label(new Rect(0, sh - 18f, sw, 16f),
                      "Rockets kill bears  |  Bears hunt all players", _labelStyle);
        }

        private void DrawCornerBracket(
            float x, float y, float size, float thick,
            bool leftSide, bool topSide, Color c)
        {
            GUI.color = c;
