...
{
    /// Target picker for the Orb Bomber. Includes the local player so a solo
    /// session can send the orb after themselves.
    public class OrbBomberOverlay : MonoBehaviour
    {
        public static OrbBomberOverlay Instance { get; private set; }
        ...
        private void OnDestroy()
        {
            if (Instance == this)
                Instance = null;
            Destroy(_panelBgTex);
            Destroy(_selectOnTex);
            Destroy(_selectHovTex);
            Destroy(_cancelBgTex);
            Destroy(_cancelHovTex);
        }
        ...
        private void DrawChooser()
        {
            ...
            GUI.Label(new Rect(px, cy, PanelWidth, HeaderH), "Orb Bomber — Choose Target", _titleStyle);
            cy += HeaderH;

            GUI.Label(
                new Rect(px, cy, PanelWidth, TipH),
                "Select a player to send the orb after, including yourself",
                _tipStyle
            );
            cy += TipH + Padding;
        ...
        private void EnsureStyles()
        {
            ...
            _panelBgTex = Tex(new Color(0.05f, 0.08f, 0.16f, 0.93f));
            _selectOnTex = Tex(new Color(0.20f, 0.45f, 0.75f, 1.00f));
            _selectHovTex = Tex(new Color(0.35f, 0.62f, 0.92f, 1.00f));
